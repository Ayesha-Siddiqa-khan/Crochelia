...
| 3 | install-nginx.sh | nginx | All EC2 instances | nginx | - | nginx |
| 4 | common-setup.sh | kubernetes-common | All EC2 instances | containerd, kubelet, kubeadm, kubectl | - | containerd, kubelet |
| 5 | control-plane-setup.sh | kubernetes-control-plane | Kubernetes master / control plane only | kubeadm, kubectl | - | kubelet |
| 6 | worker-join-setup.sh | kubernetes-worker-join | Kubernetes worker nodes only | kubeadm, kubectl | - | kubelet |
...
```bash
for cmd in curl wget unzip git helm nginx containerd kubelet kubeadm kubectl; do
...
```bash
dpkg -l | grep -E "curl|wget|unzip|git|ca-certificates|gnupg|helm|nginx|containerd.io|kubelet|kubeadm|kubectl" || echo "No matching packages found"
```

### Package list check (Amazon Linux / RHEL)

```bash
rpm -qa | grep -E "curl|wget|unzip|git|ca-certificates|gnupg|helm|nginx|containerd.io|kubelet|kubeadm|kubectl" || echo "No matching packages found"
```
